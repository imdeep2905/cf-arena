    ...
    const fetchRoom = async () => {
      ...
      if (status === "OK") {
        ...
        if (x["data"]["status"] === "OK") {
          ...
          let problemList = await axios.get(
            `http://127.0.0.1:8000/arena/create_problems?cf_handle1=${handle}&cf_handle2=${user2}`
          );
  ...
  return (
    <div>
      <div class="main">
        ...
        <div class="form">
          ...
          <div className="buttons">
            ...
            <button onClick={joinRoom} class="button button-dark">
              Join
            </button>
          </div>
        </div>
      ...
      <div class="rules">
        <h2>Rules</h2>
        <h3>
          1. For healthy competition you can turn off "Show tags for unsolved
          problems" in your codeforces settings.
        </h3>
        <h3>
          2. There will be five problem which is not been attempted by either of
          the participants.
        </h3>
        <h3>
          3. Each problem carries points associated with it (for e.x. 100, 200,
          etc..). First person to submit any problem gets the points.
        </h3>
        <h3>
          4. Once the points are claimed those problem's points can't be claimed
          by another particiapnt. So, you better hurry up.
        </h3>
        <p>
          There are 1500 points total, so the first person to get 800 or more
          points will be the winner. What strategies would you try? Start with A
          and B, then jump to E if you get them both? Or start with E, and try
          to win outright with either C or D afterwards? Read all the problems
          first and then choose a strategy based on what your opponent solves
          first?{" "}
        </p>
      </div>
    </div>
